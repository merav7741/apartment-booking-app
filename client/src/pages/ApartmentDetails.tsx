  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={headerSectionStyle}>
        ...
        <p style={{ color: '#6b7280' }}>📍 {apartmentLocation} • {apartment.address}</p>
      ...
      <div style={mainGridStyle}>
        ...
        <div>
          ...
          <button type="button" onClick={downloadApartmentDetails} style={downloadBtnStyle}>📥 הורד מפרט</button>

          <div style={bookingCardStyle}>
            <div style={{ marginBottom: '20px' }}>
              <span style={{ fontSize: '28px', fontWeight: 'bold' }}>₪{apartmentPrice}</span>
              <span style={{ color: '#6b7280' }}> / לילה</span>
            </div>

            {!isAuthenticated ? (
              <button type="button" onClick={() => navigate('/login')} style={primaryBtnStyle}>התחבר להזמנה</button>
            ) : (
              <>
                <button type="button" onClick={() => navigate(`/booking/${id}`)} style={primaryBtnStyle}>
                  🗓️ בצע הזמנה
                </button>
