...
// וודאי שהפונקציות האלו קיימות ב-apartmentSlice שלך
...
export default function EditApartment() {
  ...
  const apartmentToEdit = useAppSelector(state => 
    state.apartments.myApartments.find(a => a._id === id)
  );
  ...
  useEffect(() => {
    if (apartmentToEdit) {
      setFormData({ ...apartmentToEdit });
    } else {
      // אם הדירה לא בסטור, נטען את כולן מחדש
      dispatch(fetchMyApartments());
    }
  }, [apartmentToEdit, dispatch]);
